...
def kalman_filter_step(belief, control, measurement):
    ...
    c = 1.0
    # --->>> Put your code here.
    new_mu = a * belief.mu + control.mu
    new_sigma_sqaure = a**2 * belief.sigma2 + control.sigma2

    # Prediction.
    prediction = Density(new_mu, new_sigma_sqaure)  # Replace

    # Correction.
    K =  c * new_sigma_sqaure / (c * c * new_sigma_sqaure + measurement.sigma2)                #Kalman Gain
    mu = new_mu + K * (measurement.mu - c * new_mu)
    sigma_square = (1 - K * c) * new_sigma_sqaure

    correction = Density(mu, sigma_square)
    return (prediction, correction)
