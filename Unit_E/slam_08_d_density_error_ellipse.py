...
class ParticleFilter:
    ...
    def predict(self, control):
        ...
        left_var = (self.control_motion_factor * left)**2 + (self.control_turn_factor * (left-right))**2
        right_var = (self.control_motion_factor * right)**2 + (self.control_turn_factor * (left-right))**2
        new_particles = []
        for p in self.particles:
            sample_l = random.gauss(left, sqrt(left_var))
            sample_r = random.gauss(right, sqrt(right_var))
    ...
    def probability_of_measurement(self, measurement, predicted_measurement):
        ...
        distance, alpha = measurement
        distance_pred, alpha_pred = predicted_measurement
        ...
        distance_diff = distance-distance_pred
        alpha_diff = (alpha-alpha_pred+pi) % (2*pi) - pi
        proab_dist = normal_dist.pdf(distance_diff, 0, self.measurement_distance_stddev)
        proab_alpha = normal_dist.pdf(alpha_diff, 0, self.measurement_angle_stddev)        
        return proab_dist*proab_alpha  # Replace this.
    ...
    def get_mean(self):
        ...
        mean_x, mean_y, vector_x, vector_y = 0.0, 0.0, 0.0, 0.0
        for p in self.particles:
            mean_x += p[0]
            mean_y += p[1]
            vector_x += cos(p[2])
        ...
        mean_x = 1/M * (mean_x)
        mean_y = 1/M * (mean_y)
        mean_heading = atan2(vector_y, vector_x)
