...
class Particle:
    ...
    def h_expected_measurement_for_landmark(self, landmark_number,
        ...
        return np.array([r, alpha])  # Replace this.
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        V = np.dot(H, np.dot(self.landmark_covariances[landmark_number], H.T))
        Ql = V + Qt_measurement_covariance  # Replace this.
        return (H, Ql)
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        det = np.linalg.det(Ql)
        likely_prob = (1/(2*pi*sqrt(det))) * (exp(-0.5 * np.dot(delta_z.T, np.dot(np.linalg.inv(Ql), delta_z)))) 
        return likely_prob # Replace this..
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        scanner_pose = (self.pose[0] + cos(self.pose[2]) * scanner_displacement,
                        ...
                        self.pose[2])
        # --->>> Insert your previous code here.
        x_m, y_m = LegoLogfile.scanner_to_world(scanner_pose, measurement_in_scanner_system)
        H = self.dh_dlandmark(self.pose, (x_m, y_m), scanner_displacement)
        H_inv = np.linalg.inv(H)
        covariance = np.dot(H_inv, np.dot(Qt_measurement_covariance, H_inv.T))
        self.landmark_positions.append(np.array([x_m, y_m]))
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        Qt_measurement_covariance,scanner_displacement) # --->>> Replace this by
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for index in xrange(self.number_of_landmarks()):
            r, alpha = self.h_expected_measurement_for_landmark(index, scanner_displacement)  
            if (alpha >= min_b and alpha <= max_b):
                self.landmark_counters[index] -= 1
    ...
    def remove_spurious_landmarks(self):
        ...
        new_position = []
        for index in xrange(self.number_of_landmarks()):
            if (self.landmark_counters[index] >= 0):
                new_counter.append(self.landmark_counters[index])                    
                new_position.append(self.landmark_positions[index])
                new_covariace.append(self.landmark_covariances[index])
        self.landmark_positions = new_position
        self.landmark_covariances = new_covariace
        self.landmark_counters = new_counter        
