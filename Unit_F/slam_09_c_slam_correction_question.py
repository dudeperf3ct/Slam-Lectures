...
class ExtendedKalmanFilterSLAM:
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        x, y = initial_coords
        landmarks = array([x, y])
        new_state = hstack((self.state[:, ], landmarks[:, ]))
        self.state = new_state

        dim_x, dim_y = self.covariance.shape
        covariance_new = zeros((dim_x+2, dim_y+2))
        covariance_new[0:dim_x, 0:dim_y] = self.covariance
        covariance_new[dim_x:, dim_y:] = diag([1e10, 1e10])
    ...
    def correct(self, measurement, landmark_index):
        ...
        new_H = zeros((2,3+2*N))
        new_H[0:2,0:3] = H3
        new_H[0:2, 3+2*landmark_index : 3+2*landmark_index+2] = -new_H[0:2,0:2]
